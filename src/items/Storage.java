...
public class Storage {
	...
	public void addItem( ItemStack itemStack ) {
		...
		try {
			if (hasMaxWeight() && itemStack.getItem() != null) {
	...
	public void display() {
		VaultLogger.say("\n--------------------------------------------------------------------");
		VaultLogger.say("Backpack: ");
		VaultLogger.say(this.toString());
		VaultLogger.say("--------------------------------------------------------------------");
	}
	...
	public String displayWithValue( int playerLuck, int playerInt ) {
		if (itemStacks.isEmpty()) {
			...
			if (playerLuck > 0) {
				for (ItemStack itemStack : itemStacks) {
					int value = (int) ((0.5 + 0.02 * (playerInt + playerLuck))
					        * (itemStack.getItem().getProperties().get("value")));
					content += "- " + itemStack.getItem().getName() + " : " + itemStack.getAmount() + " at " + value
					        + " gold coins each\n";
				}
			} else {
				for (ItemStack itemStack : itemStacks) {
